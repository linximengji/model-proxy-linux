...
def _prepend_to_first_text(content: list, attribution: str) -> None:
    """Prepend attribution to the first text block in-place.

    Merges into the first text block so the attribution renders as part of
    the reply body (not a trailing standalone block that gets shown as a
    separate last message). If no text block exists, insert one at index 0.
    """
    for block in content:
        if isinstance(block, dict) and block.get("type") == "text":
            block["text"] = attribution + block.get("text", "")
            return
    content.insert(0, {"type": "text", "text": attribution})
    ...
    try:
        ...
        if used_route["provider"] in ("deepseek", "anthropic"):
            ...
            if _reason == "prompt-bypass" and resp.status_code == 200:
                ...
                _prepend_to_first_text(raw.setdefault("content", []), attribution)
        ...
        if _reason == "prompt-bypass":
            _prepend_to_first_text(result.setdefault("content", []),
                                   _inject_attribution(used_model))
                        ...
                        async for line in resp.aiter_lines():
                            ...
                            if s.startswith("data: ") and '"usage"' in s:
                                try:
                                    ...
                                    pass
                            # Inject attribution as a prefix into the first text_delta,
                            # so it renders at the reply head inside the same block stream
                            # (not as a trailing standalone block that shows as a separate
                            # last message and obscures the body).
                            if not attr_injected and attr_prefix and s.startswith("data: "):
                                try:
                                    ev = json.loads(s[6:])
                                    if ev.get("type") == "content_block_delta":
                                        delta = ev.get("delta", {})
                                        if delta.get("type") == "text_delta" and delta.get("text"):
                                            delta["text"] = attr_prefix + delta["text"]
                                ...
                                if ti is None:
                                    ...
                                    if _reason == "prompt-bypass":
                                        yield sse_encode("content_block_delta", {
                                            ...
                                            "delta": {"type": "text_delta", "text": _inject_attribution(model_name)},
                                        })
                                yield sse_encode("content_block_delta", {
                                    "type": "content_block_delta", "index": ti,
    ...
    try:
        ...
        if _reason == "prompt-bypass" and body_json.get("choices"):
            ...
            for c in body_json["choices"]:
                ...
                c["message"]["content"] = attr + existing
                ...
                async with http_client.stream(**kwargs) as resp:
                    ...
                    async for chunk in resp.aiter_bytes():
                        ...
                        if processed:
                            ...
                            # the same index concatenate in order).
                            if not attr_injected and attr_prefix and has_content:
                                attr_chunk = {
                                    "choices": [{"delta": {"content": attr_prefix}, "index": 0, "finish_reason": None}]
                                }
